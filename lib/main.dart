...
Map<String, String> localesComplete = {
  "af":"Afrikaans",
...
void main() async {
  final localesCode = localesComplete.keys;
  for (final locale in localesCode) {
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Flutter Locale'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      List? languages = localesComplete.keys.toList();
      langs = languages;
      setState(() {});
    });
  }

  Future<void> _exportToText(BuildContext context) async {
    if (langs == null) return;
    var textToCopy = '';
    final dateA = DateTime.fromMicrosecondsSinceEpoch(1138763045);
    final dateB = DateTime.fromMicrosecondsSinceEpoch(1138806245);
    for (final code in langs!) {
      try {
        var format = DateFormat('EEE, MMM d, yyyy hh:mm:ss a', code);
        var dateStringA = format.format(dateA);
        var dateStringB = format.format(dateB);
        textToCopy += '$code \n';
        textToCopy += '$dateStringA \n';
        textToCopy += dateStringB;
        textToCopy += '\n\n';
      } catch (ex) {}
    }
    ...
    return Scaffold(
      ...
      body: Center(
        child: langs == null
            ...
            itemBuilder: (ctx, idx) {
              try {
                final dateA = DateTime.fromMillisecondsSinceEpoch(1138763045);
                final dateB = DateTime.fromMillisecondsSinceEpoch(1138806245);
                final code = langs![idx];
                var format = DateFormat('EEE, MMM d, yyyy hh:mm:ss a',code);
                var dateStringA = format.format(dateA);
                var dateStringB = format.format(dateB);
                return Column(
